...
# First page layout: User input page
def page1_layout():
    return html.Div([
        ...
        html.Label('Enter your budget:'),
        dcc.Input(id='budget-input', type='number', placeholder='Your Budget'),

        html.Label('Select Windows:'),
        dcc.Dropdown(
            id='windows-dropdown',
            options=[{'label': val, 'value': val} for val in df['input|Opt-Windows'].unique()],
            value=df['input|Opt-Windows'].unique()[0]
        ),
        html.Label('Select Above Grade Wall:'),
        dcc.Dropdown(
        ...
        html.Label('Select Heating/Cooling:'),
        dcc.Dropdown(
        ...
        html.Button('Submit and See Recommendations', id='submit-button', n_clicks=0),
        dcc.Link('Go to Recommendations', href='/recommendations')
    ])

# Second page layout: Recommendations and Graphs page
def page2_layout():
    return html.Div([
        html.H1('Recommendations and Savings'),
        html.Div(id='output-container'),
        html.Div(id='recommended-upgrades', children='Loading recommended upgrades...'),
        dcc.Graph(id='savings-graph'),
        dcc.Link('Go back to Input Page', href='/')
    ])
...
# Function to calculate the best upgrades (same as in your original code)
    ...
    for component in components:
        ...
        for upgrade in df[component].unique():
            if upgrade != current_value:
                ...
                if yearly_saving > 0 and saving_15_years > 0 and net_saving > best_saving:
                    ...
                    upgrade_cost_new = upgrade_cost
                    yearly_new = yearly_saving
                    yearly15_new = saving_15_years
                    upgrade_diff_new = upgrade_diff
        ...
        if best_upgrade is not None:
            results.append({
                ...
                'Upgrade Cost': upgrade_cost_new,
                'Yearly Saving': yearly_new,
                'Saving 15 Years': yearly15_new,
                'Net Saving': best_saving,
                'Upgrade diff': upgrade_diff_new
            })
        else:
            results.append({
                'Component': component,
...
# Callback to update the recommendations and graphs on the second page
@app.callback(
        ...
        for _, row in best_upgrades_df.iterrows():
            if row['Best Upgrade'] == 'No Upgrade Found':
                ...
                recommended_text_elements.append(
                    html.P(
                        f"For {row['Component']} we recommend switching from {row['Current Value']} (Cost: {row['Current Cost']}) to {row['Best Upgrade']} (Cost: {row['Upgrade Cost']}) which saves {row['Yearly Saving']:.2f} CAD annually and {row['Saving 15 Years']:.2f} CAD in 15 years, with a net saving of {row['Net Saving']}."
                    )
                )
