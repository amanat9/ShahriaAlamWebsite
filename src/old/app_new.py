...
# Logging the data to verify columns are loaded correctly
...
def find_best_upgrades(df, user_selections):
    ...
    for component in components:
        ...
        # Log the data if no data found for current selections
        if current_cost_row.empty:
            logging.debug(f"No data found for current selections: {user_selections}")
            continue
        
        logging.debug(f"Current cost row: {current_cost_row}")
        ...
        for upgrade in df[component].unique():
            if upgrade != current_value:
                ...
                if upgrade_row.empty:
                    logging.debug(f"No upgrade row found for {component} upgrade: {upgrade}")
                    continue
                ...
                logging.debug(f"Evaluating upgrade for {component}: {upgrade} | Net Saving: {net_saving}")
...
def update_output(n_clicks, windows, abovegradewall, heatingcooling, dhwsystem):
    if n_clicks > 0:
        ...
        current_cost = df[
            ...
            (df['input|Opt-DHWSystem'] == dhwsystem)
        ][['cost-estimates|byAttribute|Opt-Windows', 
        ...
        current_util_bill_net = df[
            (df['input|Opt-Windows'] == windows) & 
            (df['input|Opt-AboveGradeWall'] == abovegradewall) & 
            (df['input|Opt-Heating-Cooling'] == heatingcooling) & 
            (df['input|Opt-DHWSystem'] == dhwsystem)
        ]['output|Util-Bill-Net'].mean()
        ...
        return (
            f"Current Cost: {current_cost:.2f} CAD, Current Util Bill (15 years): {(current_util_bill_net * 15):.2f} CAD",
