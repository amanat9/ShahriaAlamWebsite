        ...
        for _, row in best_upgrades_df.iterrows():
            if row['Best Upgrade'] == 'No Upgrade Found':
                ...
                recommended_text_elements.append(
                    html.P(
                        f"For {row['Component']} we recommend switching from {row['Current Value']} value {row['Current Cost']} to {row['Best Upgrade']} value {row['Upgrade Cost']} \n which saves {row['Yearly Saving']:.2f} CAD annually and {row['Saving 15 Years']:.2f} CAD in 15 years and net saving {row['Net Saving']}."
                    )
                )
